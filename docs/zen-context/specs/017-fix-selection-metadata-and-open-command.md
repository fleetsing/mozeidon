...
This is a latent bug that predates this spec; it was not triggered by anything in this branch or in spec 016, and most likely started manifesting after a Raycast platform update began exercising StrictMode-style double effect invocation for extension commands (the user observed the failure begin with no local code or config changes). Bundled into this branch/PR since it was found while verifying it and blocks that verification, even though it isn't part of this spec's original scope.

Initial fix attempt reset `cancelledRef.current = false` at the top of each `useEffect` invocation. Code review correctly flagged that this reintroduces a race under StrictMode's double invocation: the *first* invocation's `refresh()` can resume after `await ensureFirefoxRunning()` and observe `cancelledRef.current === false` (reset by the *second* invocation), then commit stale state after having been "cleaned up." Replaced with a monotonic per-refresh token (`latestRequestRef`): each `refresh()` call captures the current counter value as its own request id and checks it against the live counter before every state commit; each effect's cleanup increments the counter, which invalidates exactly the in-flight request(s) from that invocation without touching a request captured by a later invocation. This also fixes a smaller pre-existing gap in the original code: a manual `refresh()` call (e.g. after closing a tab) now correctly invalidates any still-in-flight prior refresh, which a shared boolean never did.
