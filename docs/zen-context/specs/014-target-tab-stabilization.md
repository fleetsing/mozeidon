...
- 2026-09-11: Manual Zen verification completed against a real Zen Browser session, covering all 7 scenarios in "Manual Verification With Zen": active tab (no target), non-active tab with `restoreFocus: true` (tab briefly opens then focus returns to the original tab), non-active tab with `restoreFocus: false` (focus stays on the target tab), closed-tab target (`tab_not_found`), a non-focused window's tab (correct window/tab focused), a restricted page (clear result, no fabricated content), and a batch of sequential targeted reads (consistent every time). Phase 1 is considered verified end-to-end.
- 2026-09-11: Phase 2 implemented. Added `mozeidon context tab --tab-id <id> --window-id <id>` (CLI), which reads the target tab directly by ID via the add-on's existing `browser.tabs.executeScript(tabId, ...)` call — no native messenger change, the target rides as an additive `target` field inside the existing `get-context` IPC request. No polling/retry/mismatch machinery is needed at the CLI/add-on layer, since reading by explicit tab ID cannot silently read the wrong tab the way switch-then-trust could; the only failure mode is `tab_not_found` (tab or window doesn't exist). Raycast's `zen_get_tab_content` now tries this direct read first (`activation.strategy: "direct"`, `focusChanged: false`, no visible tab switch) and falls back to Phase 1's focus-then-read only when the installed CLI predates `context tab` (detected via the existing `isContextCommandUnavailableError` check). A direct-read identity mismatch (CLI/add-on version skew) fails closed as `target_tab_mismatch` without falling back, since a stale add-on wouldn't be fixed by switching strategies.
- 2026-09-12: Phase 2 manually verified against a real Zen Browser session via `mozeidon context tab --tab-id <id> --window-id <id>` directly. Confirmed: `tab.active: false` with `window.focused: true` (background tab read with no switch), `contentSource: "document"`/`domRead: true` with full real page content (not metadata-only), and correct `extraction.target` echoing the request. One run against a specific tab returned `injection_unavailable`/`dom_content_unavailable` warnings with a metadata-only fallback; a page refresh in that tab resolved it on retry. This was a one-off content-script/injection staleness issue with that tab, not a Phase 2 defect — the add-on correctly degraded to the existing Spec 013 metadata-only fallback rather than failing hard or fabricating content. Worth keeping in mind: direct reads depend on the target tab's content-script environment being live; a rare stale tab may need Zen-side recovery (switching to it, or a refresh) before a direct read returns real content. Phase 2 is considered verified end-to-end.
...
- ~~Can the Firefox-family add-on reliably execute context extraction against inactive tabs using the current permissions and APIs, or is temporary activation required for some pages?~~ Yes, reliably. `browser.tabs.executeScript(tabId, ...)` (Manifest V2) accepts an explicit `tabId` and runs on background tabs with no activation, given the existing `<all_urls>` host permission — confirmed by Phase 2's manual Zen verification.
- ~~Should direct targeted CLI extraction be exposed as `context tab` or as target flags on `context active`?~~ `context tab` as a dedicated subcommand, mirroring `context active` but with required `--tab-id`/`--window-id`, keeping `context active`'s existing flag surface untouched.
