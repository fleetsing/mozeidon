  ...
  try {
    ...
    if (groupTitle !== "" || groupColor !== "") {
      await browser.tabGroups.update(groupId, {
        ...
        color: (groupColor || undefined) as GroupColor | undefined,
      })
    }
    ...
    const endTime = Date.now()
    log(`ending newGroupTab with error in ${endTime - startTime} ms`)
...
export async function switchToTab(port: Port, { args }: Command) {
  try {
    ...
    const tabItems = await browser.tabs.query({ windowId })
    ...
    return await handleError(e, port)
  }
}
...
export async function closeTabs(port: Port, { args }: Command) {
  try {
    ...
    const tabItems = await browser.tabs.query({})
    for (let tab of tabItems) {
      if (!tab.id) continue
    ...
    await browser.tabs.remove(tabToCloseIds)
...
export async function updateTabs(port: Port, { args }: Command) {
  try {
    ...
    if (tabIndex !== -2) {
      const movedTabResponse = await browser.tabs.move(tabId, {
        index: tabIndex,
        windowId: windowId,
      })
